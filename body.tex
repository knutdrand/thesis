...
This linear mapping is represented in the \code{LinearRepr} class, which has an array \code{node_offsets} where \code{node_offsets[i]}=$\sum_{k=0}^i L(k)$ such that a coordinate $(v, o) \in \graphcoord$ is converted to $i \in \linearcoord$ by \code{node_offsets[v]+o}. The inverse conversion of a coordinate $i \in \linearcoord$ is done by \code{v=searchsorted(node_offset, i); o = i-node_offsets[v])}

A pileup is created in the linear coordinate space by the following method. 
...
\begin{align*}
  \delta(LP((v, o))) &= \begin{cases}
    N_s((v, o))-N_e((v, o))+D_E((v,o)), & \text{if} \, o=0\\
    0, \text{else} \end{cases}
  \\
                         \\
  N_s((v, o)) &= \size{\buildset{(X_s, E, X_e) \in Ext}{(v, o) \in X_s}}\\
  N_e((v, o)) &= \size{\buildset{(X_s, E, X_e) \in Ext}{(v, o) \in X_s}}\\
  D_E((v, o)) &=\size{\buildset{(X_s, E, X_e) \in Ext}{\est{v_0 \in V}{(v_0, v) \in E}}}\\
&\, -\size{\buildset{(X_s, E, X_e) \in Ext}{\est{v_2 \in V}{(v-1, v_2) \in E}}}
\end{align*}
