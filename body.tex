...
A \emph{Graph} object represents a graph $G = (V, E)$ and a node labelling $L: V \rightarrow \natural$ where the vertex set is assumed to be a dense set of integers starting at $1$, $V=\set{1, 2, , , N)$, and the labeling $L$ describes the sequence length of each node.
