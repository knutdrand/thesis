...
\path [->] (X1) edge[bend right=30] node {} (X);
\path [->] (X2) edge[bend right=30] node {} (X);
\path [->] (Y1) edge[bend right=30] node {} (Y);
\path [->] (Y2) edge[bend right=30] node {} (Y);
\path [->] (X) edge[bend right=30] node {} (S);
\path [->] (Y) edge[bend right=30] node {} (S);
\end{tikzpicture}
\end{document}
