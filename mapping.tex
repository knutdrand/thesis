...
\begin{figure}
  ...
  \endtikzpicture
  \caption{\TODO{labelcolumns} Illustration of backward extension using the last-first (LF) property of the FM index.
...
\begin{figure}
  ...
  \endtikzpicture
  \caption{\TODO{label seqs} SMEMs found between query $Q$ and both strands of reference $R$. Note that the MEM $(q, r, l)=(2, 0, 2)$ (CT) is not a SMEM, since it is contained in the SMEM $(q, r, l) = (2, 4, 3)$}
  \label{fig:smem}
\end{figure}
...
\emph{vg}~\cite{vg} has been at the forefront of mapping to a graph reference, showing that it can lead to better mapping accuracy than BWA-mem. \TODO{cite 7bridges}
